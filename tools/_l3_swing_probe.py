"""Deep15 + stack → one S-west → immediately N(43,42) → east."""
...
def main():
    ...
    # Now N to (43,42) / (44,42) / (42,42)
    fr15 = lock_other_ship(data["frame"], 14)
    free = count_free14(data["frame"], fr15)
    s = max(free, key=lambda w: (w[1], w[0]))
    n = min(free, key=lambda w: (w[1], w[0]))
    for ld in ((43, 42), (44, 42), (42, 42), (43, 40), (44, 40), (45, 42), (43, 44)):
        if cheb(ld, s) < 5:
            print(f"  merge {ld}")
            continue
