"""vc33 L3 clear v5: keep 15 aligned; 14=env24↔12; 11=46; restore15=env28↔34."""
from __future__ import annotations

import json
import sys
from pathlib import Path

import numpy as np
import requests

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))
from tools.ls20_online_validate import _api_key  # noqa: E402
from tools.vc33_l3_probe import (  # noqa: E402
    FIXTURE_L3_CLEAR,
    FIXTURE_L4,
    Sess,
    body_ndiff,
    enter_l3,
    gaps_on_beams,
    pads_sorted,
    plane,
    small_sprites,
    summarize,
)

OUT = ROOT / "tests" / "fixtures" / "vc33_l3_clear_hunt.json"


def pairs(g):
    gaps = {gp["color"]: gp for gp in gaps_on_beams(g)}
    out = {}
    for s in small_sprites(g):
        if not s["accent"]:
            continue
        ac = s["accent"]["color"]
        gp = gaps.get(ac)
        if not gp:
            continue
        out[ac] = {
            "acc_y": s["accent"]["y0"],
            "gap_y": gp["y0"],
            "dy": s["accent"]["y0"] - gp["y0"],
            "aligned": s["accent"]["y0"] == gp["y0"],
        }
    return out


def click_x(sess, g, x0):
    pad = next(p for p in pads_sorted(g) if p["x0"] == x0)
    xy = (int(round(pad["cx"])), int(round(pad["cy"])))
    try:
        d = sess.click(*xy)
    except requests.HTTPError as e:
        return None, g, list(xy), str(e)
    if d.get("state") == "GAME_OVER":
        return d, plane(d["frame"]), list(xy), "GAME_OVER"
    return d, plane(d["frame"]), list(xy), None


def do(sess, d, g, x0, why, steps):
    g0 = g
    d2, g2, xy, err = click_x(sess, g, x0)
    if d2 is None:
        steps.append({"x0": x0, "xy": xy, "why": why, "error": err})
        print(" ERR", why, err)
        return d, g, False, err
    bd = body_ndiff(g0, g2)
    ps = pairs(g2)
    steps.append({
        "x0": x0, "xy": xy, "why": why, "body": bd,
        "levels": int(d2.get("levels_completed") or 0),
        "state": d2.get("state"),
        "after": {c: (p["acc_y"], p["aligned"]) for c, p in ps.items()},
        "error": err,
    })
    print(f" {why:10} x{x0} body={bd} lv={steps[-1]['levels']}", steps[-1]["after"], err or "")
    return d2, g2, bd > 0, err


def ensure_15(sess, d, g, steps, limit=10):
    for _ in range(limit):
        ps = pairs(g)
        if ps.get(15, {}).get("aligned"):
            return d, g, True
        if 15 not in ps:
            return d, g, False
        want = 38 if ps[15]["dy"] > 0 else 34
        env = 24 if want == 38 else 28  # guess: 24 opens up-corridor, 28 opens down
        d, g, ok, err = do(sess, d, g, want, "15", steps)
        if err == "GAME_OVER" or int(d.get("levels_completed") or 0) >= 3:
            return d, g, False
        if ok:
            continue
        d, g, _, err = do(sess, d, g, env, "env15", steps)
        if err == "GAME_OVER":
            return d, g, False
        d, g, ok, err = do(sess, d, g, want, "15", steps)
        if not ok:
            # try other env
            other = 28 if env == 24 else 24
            d, g, _, err = do(sess, d, g, other, "env15b", steps)
            d, g, ok, err = do(sess, d, g, want, "15", steps)
            if not ok:
                return d, g, False
    return d, g, pairs(g).get(15, {}).get("aligned", False)


def ensure_14(sess, d, g, steps, limit=20):
    for _ in range(limit):
        ps = pairs(g)
        if ps.get(14, {}).get("aligned"):
            return d, g, True
        # keep 15 aligned first — seems required for env24
        d, g, ok15 = ensure_15(sess, d, g, steps, limit=6)
        if int(d.get("levels_completed") or 0) >= 3:
            return d, g, True
        if not ok15:
            print("cannot keep 15 aligned")
        d, g, ok, err = do(sess, d, g, 12, "14", steps)
        if err == "GAME_OVER" or int(d.get("levels_completed") or 0) >= 3:
            return d, g, int(d.get("levels_completed") or 0) >= 3
        if ok:
            continue
        d, g, _, err = do(sess, d, g, 24, "env24", steps)
        if err == "GAME_OVER":
            return d, g, False
        d, g, ok, err = do(sess, d, g, 12, "14", steps)
        if not ok:
            print("14 blocked even after env24; try 28 toggle")
            d, g, _, err = do(sess, d, g, 28, "env28", steps)
            d, g, _, err = do(sess, d, g, 24, "env24", steps)
            d, g, ok, err = do(sess, d, g, 12, "14", steps)
            if not ok:
                return d, g, False
    return d, g, pairs(g).get(14, {}).get("aligned", False)


def main():
    key = _api_key()
    sess = Sess(key)
    out = {}
    try:
        sess.open()
        ent = enter_l3(sess)
        d, g = ent["data"], ent["frame"]
        steps = []
        print("start", pairs(g))

        # 1) Pull 15 up with 38 a bit (optional)
        d, g, _ = ensure_15(sess, d, g, steps, limit=3)

        # 2) Bring 11 down with 46; after each, ensure_15
        for _ in range(16):
            ps = pairs(g)
            print("11-loop", ps)
            if ps.get(11, {}).get("aligned"):
                break
            if ps[11]["dy"] >= 0:
                break
            d, g, ok, err = do(sess, d, g, 46, "11", steps)
            if err == "GAME_OVER" or int(d.get("levels_completed") or 0) >= 3:
                break
            d, g, _ = ensure_15(sess, d, g, steps, limit=8)

        print("after 11", pairs(g))

        # 3) Finish 14
        d, g, ok14 = ensure_14(sess, d, g, steps)
        print("after 14", pairs(g), "ok", ok14, "lv", d.get("levels_completed"))

        # 4) Final ensure all
        d, g, _ = ensure_15(sess, d, g, steps)
        d, g, _ = ensure_14(sess, d, g, steps)

        ps = pairs(g)
        print("final pairs", ps, "lv", d.get("levels_completed"))
        if all(p["aligned"] for p in ps.values()) and int(d.get("levels_completed") or 0) < 3:
            for x0 in (12, 24, 38, 46, 34, 28, 50):
                d, g, _, err = do(sess, d, g, x0, "post", steps)
                if int(d.get("levels_completed") or 0) >= 3 or err == "GAME_OVER":
                    break

        out["steps"] = steps
        out["cleared"] = int(d.get("levels_completed") or 0) >= 3
        out["final"] = pairs(g)
        print("CLEARED", out["cleared"], out["final"])

        if out["cleared"]:
            FIXTURE_L3_CLEAR.write_text(
                json.dumps({
                    "game_id": sess.game_id,
                    "meta": {
                        "levels_completed": d.get("levels_completed"),
                        "state": d.get("state"),
                        "available_actions": d.get("available_actions"),
                        "win_levels": d.get("win_levels"),
                    },
                    "steps": steps,
                    "summary": summarize(g),
                    "frame": g.tolist(),
                }, indent=2, default=float),
                encoding="utf-8",
            )
            stale = g
            d4 = sess.action("ACTION1")
            g4 = plane(d4["frame"])
            diff = int(np.sum(stale != g4))
            FIXTURE_L4.write_text(
                json.dumps({
                    "game_id": sess.game_id,
                    "meta": {
                        "levels_completed": d4.get("levels_completed"),
                        "state": d4.get("state"),
                        "available_actions": d4.get("available_actions"),
                        "win_levels": d4.get("win_levels"),
                    },
                    "sync": {"action": "ACTION1", "pixel_diff_from_stale": diff},
                    "summary": summarize(g4),
                    "frame": g4.tolist(),
                }, indent=2, default=float),
                encoding="utf-8",
            )
            print("saved fixtures", diff)
    finally:
        sess.close()
    OUT.write_text(json.dumps(out, indent=2, ensure_ascii=False, default=float), encoding="utf-8")
    print("saved", OUT)
    return 0 if out.get("cleared") else 1


if __name__ == "__main__":
    raise SystemExit(main())
