"""From frog: only advance south wp east; avoid north-south catch (dead)."""
    ...
    for rnd in range(8):
        ...
        for ld in (
            ...
            if st == "moved":
                ...
                break
        if not moved:
            # try north east same row instead
            for ld in ((north[0] + 4, north[1]), (north[0] + 5, north[1]), (north[0] + 2, north[1])):
                if max(abs(ld[0] - south[0]), abs(ld[1] - south[1])) < 5:
                    continue
                if near_any(ld, list(fr15), cheb=5):
                    continue
                data, newc, st = move_wp(sess, data, north, ld, fr15)
                print(f"  N {north}->{ld} {st}")
                if st == "dead" or data.get("state") == "GAME_OVER":
                    print("DEAD N")
                    return
                if st == "moved":
