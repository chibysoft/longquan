"""From N44 pose (44,36)+(48,42) d14=34 bud≈10: scan improving hops."""
...
# Known dead/noop under Ne pose — still try some; N44 may differ
CANDS = []
# from north (44,36)
for t in (
    (46, 36),
...
def main():
    ...
    for who, tgt in CANDS:
        ...
        if cheb(tgt, other) < 5 or near_any(tgt, list(fr15), cheb=5):
            print("skip merge/near15", flush=True)
            continue
        ...
        if d1 < d0:
            print(
                f"IMPROVED d14 {d0}->{d1} bud={step_budget(data['frame'])}",
                flush=True,
            )
            hits.append((who, tgt, newc, d0, d1, step_budget(data["frame"]), free1))
        elif d1 == d0:
            print(f"FLAT bud={step_budget(data['frame'])}", flush=True)
        else:
            print(f"WORSE", flush=True)

    print("\n===== HITS =====", flush=True)
    for h in hits:
        print(h, flush=True)
    if not hits:
        print("none — but N44 still saves bud vs Ne", flush=True)
