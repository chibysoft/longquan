"""g50t L4: while HOLD on tip (e8=36 no A5), try west along snake; also prelatch top-right.
...
OUT = ROOT / "tests/fixtures/g50t_l4_hold_west.json"
...
TIP_STEP = [2, 2, 2, 4, 2]
# spawn → (40,22) → U U → (40,10) → R R → (52,10) → D…
TO_TOP_RIGHT = [2, 2, 4, 4, 1, 1, 4, 4, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3]
...
def main():
    ...
    def slog(label, guid, g, prev, seq):
        ...
        for a_ in seq:
            ...
            if de < 0 or (de > 0 and e8(g)["n"] not in (36, 52)):
                p("  *** e8", einfo(g))
            if de < 0 and pos != before:
                p("  *** MOVE+SHRINK")
    ...
    # A: hold tip, try L west / D / R
    guid, g, prev, d = enter_l4()
    guid, g, prev, d, ok, dead = play(guid, g, prev, TIP_STEP, quiet=False)
    p("HOLD", (prev["cx"], prev["cy"]), einfo(g))
    guid, g, prev, d, dead = slog("HW", guid, g, prev, [3, 3, 3, 3, 2, 2, 4, 4, 1, 3, 5])

    # B: no tip, top-right path (U from 40,22)
    p("## prelatch top-right")
    guid, g, prev, d = enter_l4()
    guid, g, prev, d, dead = slog("TR", guid, g, prev, TO_TOP_RIGHT)
