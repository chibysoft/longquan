...
Builds a synthetic AR25 L1 frame by hand (no engine, no network), then runs the
full closed-book loop and asserts the config + actions match the known optimum.
"""
import sys
import os
...
def _build_l1_frame():
    ...
    for gy in range(21):
        put(10, gy, 10)  # vertical mirror axis x=10
    for (dx, dy) in [(0, 0), (1, 0), (2, 0), (2, 1), (2, 2)]:  # "┐" piece
        put(6 + dx, 5 + dy, 5)
...
def test_search_finds_known_optimum():
    obs = perceive(_build_l1_frame(), steps_left=64)
    configs = solve_configs(obs, obs.line_coords, mirror.cover, mirror.backproject)
...
def test_loop_replays_ok():
    ...
    result = solve(obs, obs.line_coords, mirror.cover, mirror.backproject,
